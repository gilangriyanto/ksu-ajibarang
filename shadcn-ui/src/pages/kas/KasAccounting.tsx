...
// Interface untuk journal entry
interface JournalEntry {
  id: string;
  kas_id: number; // ← PENTING!
  date: string;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>Tambah Jurnal Manual - Kas {kasId}</DialogTitle>
          <DialogDescription>
            Buat jurnal entry manual untuk transaksi khusus di Kas {kasId}.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="journal-date" className="text-right">
              Tanggal
            </Label>
            <Input id="journal-date" type="date" className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="journal-ref" className="text-right">
              Referensi
            </Label>
            <Input
              id="journal-ref"
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="journal-desc" className="text-right">
              Deskripsi
            </Label>
            <Textarea
              id="journal-desc"
                ...
                <div key={index} className="grid grid-cols-12 gap-2">
                  <Select>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="1-1101">
                        1-1101 - Piutang Anggota
                      </SelectItem>
                      <SelectItem value="2-2001">
                        2-2001 - Simpanan Anggota
                      </SelectItem>
                      <SelectItem value="4-1001">
                        4-1001 - Pendapatan Jasa
                      </SelectItem>
                      <SelectItem value="5-1001">
                        5-1001 - Beban Operasional
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <Input
...
export default function KasAccounting() {
  ...
  // Mock data - SEMUA JURNAL dari SEMUA KAS
  const allJournalEntries: JournalEntry[] = [
    // Kas 1
    {
      id: "JU001",
      kas_id: 1, // ← FILTER BY KAS_ID
      date: "2024-01-15",
      type: "general",
    ...
    // Kas 3
    {
      ...
      description: "Setoran simpanan Budi Prasetyo",
      entries: [
    ...
    },
    {
      ...
      entries: [
        ...
        {
          account: "1-2002",
          accountName: "Akm. Penyusutan Aset Tetap",
          debit: 0,
          credit: 3125000,
        },
      ],
    ...
    // Kas 2
    {
      ...
      date: "2024-01-18",
      type: "general",
      reference: "Pembelian-001",
      description: "Pembelian barang logistik",
      entries: [
        {
          account: "1-1301",
      ...
      totalCredit: 5000000,
      status: "posted",
      createdBy: "Admin Kas 2",
      source: "manual",
    },
  ];

  // ✨ MAGIC FILTER - Hanya tampilkan jurnal untuk kas ini
  const journalEntries = allJournalEntries.filter(
    (journal) => journal.kas_id === kasId
  );
  ...
  const handleEditJournal = (journal: JournalEntry) => {
    toast.info(`Edit jurnal ${journal.id}`);
  };

  const handleDeleteJournal = (journal: JournalEntry) => {
    toast.success(`Jurnal ${journal.id} dihapus`);
  };
  ...
  // Calculate summary - HANYA untuk kas ini
  const journalSummary = {
    totalEntries: journalEntries.length,
    totalDebit: journalEntries.reduce(
      (sum, journal) => sum + journal.totalDebit,
      0
    ),
    totalCredit: journalEntries.reduce(
      (sum, journal) => sum + journal.totalCredit,
      0
    ),
    autoGenerated: journalEntries.filter((j) => j.createdBy === "System Auto")
      .length,
    manualEntries: journalEntries.filter((j) => j.createdBy !== "System Auto")
      .length,
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Kelola jurnal keuangan Kas {kasId}
            </p>
          </div>
          <Button
        ...
        <Tabs defaultValue="all" className="space-y-4">
          ...
          <TabsContent value="all" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Daftar Jurnal Kas {kasId}</CardTitle>
                <CardDescription>
                  Menampilkan {filteredJournals.length} dari{" "}
                  {journalEntries.length} jurnal
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      {filteredJournals.length === 0 ? (
                        <tr>
                          <td colSpan={9} className="text-center py-12">
                            ...
                            <p className="text-gray-500">
                              Belum ada jurnal untuk Kas {kasId}
                            </p>
                          </td>
                        </tr>
                      ) : (
                        filteredJournals.map((journal) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-4">
                              <Badge
                                className={getJournalTypeColor(journal.type)}
                              >
          ...
          <TabsContent value="general">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Jurnal untuk transaksi umum dan manual entry
                </CardDescription>
              </CardHeader>
              <CardContent className="py-12 text-center">
                <p className="text-gray-500">
                  Jurnal umum akan ditampilkan di sini...
                </p>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="special">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Jurnal otomatis dari transaksi pinjaman dan simpanan
                </CardDescription>
              </CardHeader>
              <CardContent className="py-12 text-center">
                <p className="text-gray-500">
                  Jurnal khusus akan ditampilkan di sini...
                </p>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="adjustment">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Jurnal untuk penyesuaian akhir periode
                </CardDescription>
              </CardHeader>
              <CardContent className="py-12 text-center">
                <p className="text-gray-500">
                  Jurnal penyesuaian akan ditampilkan di sini...
                </p>
              </CardContent>
            </Card>
          </TabsContent>
